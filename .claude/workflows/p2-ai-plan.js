...
spring.ai.anthropic.base-url: \${AI_BASE_URL:https://token-plan.cn-beijing.maas.aliyuncs.com/apps/anthropic}；spring.ai.anthropic.api-key: \${AI_API_KEY:}；spring.ai.anthropic.chat.options.model: \${AI_MODEL:qwen3.8-flash}、temperature: 0.5、max-tokens: 2000；spring.ai.retry.max-attempts: 2；顶层 app.ai.{enabled:\${AI_ENABLED:false}, daily-generate-limit:5, journal-days:3, journal-chars-per-day:600, reflection-limit:10, reflection-chars:200, min-blocks:3, max-blocks:12}。prod profile 追加 api-key: \${AI_API_KEY}（无默认值）、app.ai.enabled: \${AI_ENABLED:true}。若 spring-ai 1.1.8 实际属性名与此有出入（base-url/重试键等），以依赖 jar 内 spring-configuration-metadata.json 为准并在返回中列出偏差。
### docker-compose.yml backend environment 追加 AI_API_KEY=\${AI_API_KEY:?required}、AI_ENABLED=\${AI_ENABLED:-true}（对齐现有 env 风格）
